...
| **크롤 이력** | `src/app/(dashboard)/crawl-history/page.tsx` ✅ 신규 |
...
| Cron 크롤 | `src/app/api/cron/crawl/route.ts` ✅ 3-batch 분할 |
| Cron 만료 | `src/app/api/cron/expire/route.ts` |
| **크롤 이력 API** | `src/app/api/crawl-history/route.ts` ✅ 신규 |
...
| 파일 | 경로 | 설명 |
|------|------|------|
| **vercel.json** | `vercel.json` (루트) | **✅ Cron 스케줄 (3-batch + expire)** |
...
│   │   └── [id]/page.tsx    — 회원 상세
│   ├── crawls/              — 크롤 모니터링
│   └── crawl-history/       — 크롤 이력 (날짜별 요약/상세+비용) ✅ 신규
├── api/
│   ├── auth/route.ts
│   ├── deals/route.ts
│   ├── merchants/route.ts
...
│   │   └── [id]/route.ts   — 상세 GET
│   ├── crawl-history/
│   │   └── route.ts         — 크롤 이력 API (날짜별 통계+비용) ✅ 신규
│   ├── ai-crawl/route.ts
│   └── cron/
│       ├── crawl/route.ts
│       └── expire/route.ts
...
- `GET /api/crawl-history` — 크롤 이력+날짜별 통계+비용 (최근 30일) ✅ 신규
- `GET /api/cron/crawl` — 일일 자동 크롤 배치 (3-batch 분할) ✅
...
### crawl_runs 테이블
```
id, connector_id, status (running/success/partial/failed),
new_count, updated_count, expired_count,
error_message, started_at, completed_at,
tokens_used INTEGER DEFAULT 0   -- ✅ 2/18 추가: AI 토큰 사용량 (비용 추적용)
```
...
- [x] Cron 자동 크롤 설정 — 23:00 KST 3-batch 분할 (vercel.json) ✅ 2/18

---

## 배치 스케줄 (어드민 앱에서 관리)
- **현재**: Vercel Cron — 매일 23:00 KST **3-batch 분할** ✅ (2/18 변경)
  - 23:00 KST (14:00 UTC): batch=1 (커넥터 1/3, ~81개)
  - 23:20 KST (14:20 UTC): batch=2 (커넥터 2/3, ~81개)
  - 23:40 KST (14:40 UTC): batch=3 (커넥터 3/3, ~80개)
  - 23:50 KST (14:50 UTC): expire (만료 딜 자동 처리)
- **안전장치**: 250초 타임아웃 (Vercel 300초 제한 전 중단)
- **vercel.json**: `C:\projects\poppon-admin\vercel.json`에 crons 배열 정의
- **추후**: 필요 시 2회(06:00+23:00) 증설
...
- **Vercel 배치 크롤 타임아웃**: 300초 제한 → ✅ 3-batch 분할 + 250초 타임아웃으로 해결 (2/18). 수동 크롤은 여전히 프론트 순차 호출 방식
...
- **Cron 3-batch 분할**: vercel.json에 crons 배열 정의, batch=1/2/3 쿼리 파라미터로 분할. 커넥터 이름순 정렬 → 일관된 배치 배정. 250초 타임아웃 초과 시 나머지 스킵
- **DealModal 스크롤**: `overflow: hidden`만으로는 스크롤 점프 발생 → `position: fixed` + `top: -scrollY` 패턴 필수. 닫힐 때 `window.scrollTo(0, scrollY)` 복원
- **crawl_runs.tokens_used**: 2/18 ALTER TABLE로 추가. 기존 데이터는 0. completeCrawlRunLog에서 저장 — save-deals.ts의 SaveResult 인터페이스에 맞춰야 빌드 통과
...
| **팝폰-로고교체+푸터+홈그리드** | **2/18** | **로고 34종 교체/복구(삼성svg→png), 푸터 사업자 정보 추가(13px+볼드), 홈 딜 섹션 limit 48(4열×2줄 꽉 참)** |
| **팝폰-Cron자동화+크롤이력** | **2/18** | **Cron 3-batch 분할(23:00/23:20/23:40 KST), 250초 타임아웃, 크롤 이력 페이지+API 신규(날짜별 요약/상세/비용), crawl_runs.tokens_used 추가, DealModal 스크롤 점프 방지(position fixed 패턴)** |
---
...
### Cron 자동 크롤 3-batch + 크롤 이력 페이지 + 모달 스크롤 수정 (2/18)
- [x] Vercel Cron 3-batch 분할 — 23:00/23:20/23:40 KST (vercel.json + cron/crawl/route.ts 전면 수정)
- [x] 배치 분할 로직 — 커넥터 이름순 정렬 후 3등분, batch=1/2/3 쿼리 파라미터
- [x] 250초 타임아웃 안전장치 — Vercel 300초 제한 전 자동 중단, 나머지는 다음 날 처리
- [x] 크롤 이력 페이지 신규 (`/crawl-history`) — 30일 날짜별 요약 + 클릭→상세 보기
- [x] 크롤 이력 API 신규 (`/api/crawl-history`) — crawl_runs 조회 + 날짜별 그룹핑 + 통계
- [x] 비용(tokens_used) 추적 — crawl_runs 테이블에 tokens_used 컬럼 추가 (ALTER TABLE)
- [x] 크롤 이력 비용 표시 — 30일 총 비용 카드, 날짜별 비용 컬럼, 상세 커넥터별 비용
- [x] 어드민 사이드바 "🕐 크롤 이력" 메뉴 추가 (dashboard layout.tsx)
- [x] DealModal 스크롤 점프 방지 — `overflow: hidden` → `position: fixed + top: -scrollY` 패턴 (모달 열릴 때 백그라운드 점프 근본 해결)

---

*마지막 업데이트: 2026-02-18 (Cron 3-batch 자동 크롤 + 크롤 이력 페이지 + DealModal 스크롤 수정)*
