...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    if (!error) {
      // ── 비밀번호 재설정 요청인지 확인 (쿠키 기반) ──
      const isPasswordReset = cookieStore.get('password_reset_pending')?.value === 'true';
      if (isPasswordReset) {
        const response = NextResponse.redirect(new URL('/auth/reset-password', requestUrl.origin));
        response.cookies.set('password_reset_pending', '', { path: '/', maxAge: 0 });
        return response;
      }
